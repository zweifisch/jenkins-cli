...
class Jenkins {
    ...
    constructor({url}){
        this.client = new Request({baseUrl: url, redirect: 'follow', follow: 3})
    }
    ...
    lastBuild(...name) {
        return this.client.get(`${name2path(...name)}/lastBuild/api/json`).json
    }

    output (...args) {
        let build = args.pop()
        return this.client.get(`${name2path(...args)}/${build}/consoleText`).text
    }
